...
import game.gameobjects.items.PowerUp;
import game.gameobjects.items.Rakh;
import game.gameobjects.items.TimeDialation;
...
public class ServerEngine implements ActionListener {
     ...
     * @param zoneNo
     */
    public void eventPlayerInZone(MutantPC player, int zoneNo) {
     ...
     * @param player
     */
    public void eventPlayerIsAttacked(MutantPC player) {
        boolean hasEgg = player.hasEgg();
     ...
     * @param item
     */
    public void eventPlayerItemPickup(MutantPC player, Item item) {
        boolean hasEgg = player.hasEgg();
        int playerIndex = level.players.indexOf(player);
        ...
        if (item instanceof Rakh) {
            ...
            if (quantity < 2 && hasEgg == true || quantity < 1 && hasEgg == false) {
                ArrayList<Item> contents = box.getContents();
     ...
     * @param the recieved client data
     */
    public void readClientData(ClientData clientData) {
     ...
     * @param gameState
     */
    public void setGameStateReference(GameState gameState) {
    ...
    /**
     * This method sets the play time.
     * @param value
     */
            ...
            if (product == 1) {
                ...
                if (!checkSpawnWallCollision(box)) {
                    if (!checkSpawnZoneCollision(box)) {
                        //System.out.println("Spawning Box");
